 ...
 *   - spawner: `get_app_info` (→ factory_canister_id) + `peek_install_token`
 *     (pre-check read-only: app_id autentico, NON brucia) + `consume_install_token`
 *     (commit: brucia il token one-time + sposta l'occupancy, solo a reinstall riuscito);
...
export const spawnerHandoffIdl = () => IDL.Service({
  ...
  peek_install_token:    IDL.Func([IDL.Vec(IDL.Nat8), IDL.Principal], [ResultText], ['query']),
