...
Reproducible source for the EasyCan sovereign apps (**vault**, **messenger**, **EasyHub**). This
repository is the **trust anchor**: anyone can rebuild a canister's WASM from a tagged commit and
obtain the same `module_hash` the Internet Computer certifies on-chain (the in-app `#verify` page
shows it live).
...
The `PACKAGE` is the crate name: `vault-canister`, `messenger-canister`, or `hub-canister` (EasyHub —
note the crate is `hub-canister` even though its release tag uses the `supercanister-hub-` prefix).

## Releases & self-upgrade
Each tag `vault-vX.Y.Z` / `messenger-vX.Y.Z` / `supercanister-hub-vX.Y.Z` triggers `.github/workflows/release.yml`, which builds
